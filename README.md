...
The business was spending over **$142K across three months** on digital advertising with no unified view of which campaigns, creative formats, or time windows were actually driving profitable returns.
...
SQL Cleaning Layer
  ...
  → raw_sales_clean (negative quantities, zero prices, missing IDs removed)
        ↓
SQL Aggregation & Privacy Firewall
...
## 🛡️ Data Governance & Privacy Firewall

> In 2026, data ethics is not a checkbox — it is a competitive differentiator.

This pipeline includes a deliberate **SQL Privacy Firewall**: `CustomerID` is used only within the cleaning and aggregation layer to count unique customers per day, then **dropped entirely** before the data reaches the reporting and visualisation layer (`sales_ads_joined_focus`).
...
│  Exposes $100K+ in reallocation opportunity             │
└─────────────────────────────────────────────────────────┘
```
